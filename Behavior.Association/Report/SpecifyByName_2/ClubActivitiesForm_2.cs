...
{
    public partial class ClubActivitiesForm_2 : BaseForm
    {
        public const string ConfigName = "社團活動點名單設定檔_Word_2";

        //UDT物件
        private AccessHelper _accessHelper = new AccessHelper();
        //上課地點清單
        Dictionary<string, string> DicAddress;
        ...
        void BGW_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            //取得學生課程資料
            CourseDetail = new GetCourseDetail(AssnAdmin.Instance.SelectedSource);

            #endregion

            #region 2012/2/22 上課地點資訊

            List<string> now = AssnAdmin.Instance.SelectedSource;
            string abc = "'" + string.Join("','", now) + "'";
            List<AssnAddress> Addresslist = _accessHelper.Select<AssnAddress>(string.Format("AssociationID in ({0})", abc));
            DicAddress = new Dictionary<string, string>();
            foreach (AssnAddress each in Addresslist)
            {
                if (!DicAddress.ContainsKey(each.AssociationID))
                {
                    DicAddress.Add(each.AssociationID, each.Address);
                }
            }
            ...
            {
                template = new Document(new MemoryStream(Properties.Resources.社團活動範本_編號));
            }
            else
            {
                if (GetCD._PrintTemp == "")
            ...
            foreach (string each in CourseDetail.SortDicCourse.Keys)
            {
                ...
                if (DicAddress.ContainsKey(each))
                {
                    name.Add("上課地點");
                    value.Add(DicAddress[each]);
                }
                ...
                name.Add("社團名稱");
                value.Add(CourseDetail.DicCourseStudent[each][0].CourseName);

                name.Add("指導老師");
                value.Add(CourseDetail.SortDicCourse[each].MajorTeacherName);
        ...
        void MailMerge_MergeField(object sender, Aspose.Words.Reporting.MergeFieldEventArgs e)
        {
            ...
            if (e.FieldName == "資料")
            {
                ...
                List<super> StudList = CourseDetail.DicCourseStudent[e.FieldValue.ToString()];

                Row refrow = builder.CurrentParagraph.ParentNode.ParentNode as Row;
                Cell SplieCell = GetMoveRightCell(refrow.Cells[0], 4);
                ...
                int NumberCount = 0;
                foreach (super each in StudList)
                {
                    NumberCount++;
                    Write(refrow.Cells[0], NumberCount.ToString());
                    Write(refrow.Cells[1], each.ClassName);
                    Write(refrow.Cells[2], each.SeatNo);
                    Write(refrow.Cells[3], each.StudentName);
